<?php
/**
 * \dimadin\WP\Plugin\CSSJSURLRewriter\Queue class.
 *
...
use dimadin\WP\Plugin\CSSJSURLRewriter\Process;
...
class Queue {
	...
	public static function process() {
		 ...
		 */
		$stored_data = apply_filters( 'cjur_pre_process_queue_stored_data', $stored_data );
		 ...
		 *                 Default 10.
		 */
		$max = apply_filters( 'cjur_max_items_in_processing_queue', 10 );
		...
		foreach ( $stored_data['queue'] as $origin_path => $path_settings ) {
			...
			if ( in_array( $path_settings['type'], [ 'network_site_url', 'network_content_url' ], true ) ) {
				$stored_data['active'][ $path_settings['type'] ] = [
				...
				try {
					 ...
					 */
					$ttl = apply_filters( 'cjur_inactive_path_ttl', DAY_IN_SECONDS, $process );
